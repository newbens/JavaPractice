...
public interface BlogMapper {
    ...
    @Transactional
    @Insert("insert into t_blog(content,title,create_time,typeId,userId,firstPicture,views,update_time,typeName,flag,description,userName)" +
            " values(#{content},#{title},#{createTime},#{typeId},#{userId},#{firstPicture},#{views},#{updateTime},#{typeName},#{flag},#{description},#{userName})")
    ...
    @Update("update t_blog set title=#{blog.title},content=#{blog.content},flag=#{blog.flag},firstPicture=#{blog.firstPicture}, " +
            "typeName=#{blog.typeName},typeId=#{blog.typeId},update_time=#{blog.updateTime},description=#{blog.description} where id=#{id}")
    ...
    @Select("select * from t_blog where content like CONCAT('%',#{search},'%')")//使用不到索引
    List<Blog> getSearch(String search);

    @Select("select * from t_blog where content like CONCAT('%',#{search},'%') and typeId = #{typeId}")//使用不到索引
